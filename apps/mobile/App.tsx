...
function AutomationManager({ api, automations, threads, categories, onClose, onChanged }: { api: BoardApi; automations: Automation[]; threads: ThreadDto[]; categories: string[]; onClose: () => void; onChanged: () => void }) {
  const [kind, setKind] = useState<"recurringMessage" | "scheduledMessage" | "calendarMessage" | "categoryPipeline">("recurringMessage");
  ...
  async function create() {
    ...
    if (kind === "recurringMessage") input = { name: name.trim(), action: { kind, threadId, prompt: prompt.trim(), everyMinutes: interval, startInMinutes: interval } };
    ...
    else if (kind === "calendarMessage") { const [hour, minute] = time.split(":").map(Number); input = { name: name.trim(), action: { kind, threadId, prompt: prompt.trim(), weekdays, minuteOfDay: hour * 60 + minute, timezoneOffsetMinutes: new Date().getTimezoneOffset() } }; }
    else input = { name: name.trim(), action: { kind, fromCategory, toCategory, afterMinutes: interval } };
    setBusy(true);
  ...
  const validSchedule = kind === "scheduledMessage" ? parseLocalDateTime(runAt) > Date.now() : kind !== "calendarMessage" || weekdays.length > 0;
...
const styles = StyleSheet.create({
  ...
  settingsRow: { minHeight: 64, marginBottom: 8, paddingHorizontal: 14, flexDirection: "row", alignItems: "center", justifyContent: "space-between", borderWidth: 1, borderColor: "#E2E4EC", borderRadius: 13, backgroundColor: "white" },
